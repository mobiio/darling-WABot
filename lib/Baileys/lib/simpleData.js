...
var messageKeys = Object.keys(mek.message) ? Object.keys(mek.message) : undefined;
if (messageKeys === undefined) resolve(false)
    
    for( var i = 0; i < messageKeys.length; i++){ 
                                   
        if ( messageKeys[i] === "messageContextInfo") { 
            messageKeys.splice(i, 1); 
            break
        }
    }
    console.log(messageKeys)
    let messageType = messageKeys[0];
    let composeData = {};
